 ...
 * Load from ADUser
 */
public class OpenbravoFindByName extends DefaultHashVariablePlugin {
    public final static String LABEL = "Openbravo Find By Name";

    @Override
    public String getPrefix() {
        return "openbravoFindByName";
    }
    ...
    public String processHashVariable(String key) {
        ...
        final String field = Arrays.stream(split).skip(1).findFirst().orElse("id");
        LogUtil.info(getClassName(), "field [" + field + "]");
        final Matcher matcher = Pattern.compile("(?<=\\[).*(?=])").matcher(key);
        final String searchName = matcher.find() ? matcher.group() : "";
        ...
        try {
            ...
            final Map<String, Object>[] result = obService.get(baseUrl, tableEntity, username, password, Collections.singletonMap("name", searchName));
            LogUtil.info(getClassName(), "result.length [" + result.length + "]");
            final String value = Optional.of(result)
                    .stream()
                    .flatMap(Arrays::stream)
                    .peek(m -> LogUtil.info(getClassName(), "map [" + m + "]"))
                    .findFirst()
            ...
            LogUtil.error(getClassName(), e, "Error loading table [" + tableEntity + "] name [" + searchName + "] field [" + field + "]");
            return "";
        }
    }
    ...
    public Collection<String> availableSyntax() {
        ...
        list.add(getPrefix() + ".TABLE_ENTITY[SEARCH_NAME].GET_FIELD");
        return list;
    }
